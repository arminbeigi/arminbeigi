...
- **آپلود فایل + لینک از ما (پشت‌صحنه)**: فایل PDF روی هاست مرجع
  ...
  `yarapro.ir/factor/shofazh-1024-k7m9`
  (کلمه‌ی factor + نام فروشنده + شماره فاکتور + کد تصادفی ۴ حرفیِ غیرقابل‌حدس).
...
- **افزونه‌ی سرور**: `store/yara-filestore/yara-filestore.php`
  ...
  - مسیر: `GET /factor/{slug}` فایل PDF را سرو می‌کند (و `/f/` برای سازگاری)
